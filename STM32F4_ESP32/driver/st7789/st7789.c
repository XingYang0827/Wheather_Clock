...
static void st7789_write_single_ascii(uint16_t x, uint16_t y, char ch, uint16_t color, uint16_t bg_color, const font_t *font)
{
    ...
    GPIO_SetBits(DC_PORT, DC_PIN); 	// data mode
		...
		const uint8_t* model = font->ascii_model + (ch - ' ') * font->height * bytes_per_row;
...
static void st7789_write_single_chinese(uint16_t x, uint16_t y, char ch, uint16_t color, uint16_t bg_color, const font_t *font)
{
	if (!in_screen_range(x, y, x+font->width-1, y+font->height-1)) return;
...
void st7789_draw_image(uint16_t x, uint16_t y, const image_t *image)
{
    ...
    st7789_set_range_and_gram(x, y, x + image->width - 1, y + image->height - 1);
