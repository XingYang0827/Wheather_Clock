...
// ---- read helper ----
static bool i2c_read_bytes(uint8_t *data, uint32_t n)
{
    if (n == 0) return true;
    ...
    if (n == 1) {
        ...
        I2C_AcknowledgeConfig(I2C2, DISABLE);
        I2C_GenerateSTOP(I2C2, ENABLE);
        I2C_WAIT_FLAG(I2C_FLAG_RXNE, 1000);
    ...
    if (n == 2) {
        // 2 bytes
        I2C_NACKPositionConfig(I2C2, I2C_NACKPosition_Next); // POS=1
        I2C_AcknowledgeConfig(I2C2, DISABLE);
        (void)I2C2->SR1; (void)I2C2->SR2; // clear pending state

        I2C_WAIT_FLAG(I2C_FLAG_BTF, 1000);
        I2C_GenerateSTOP(I2C2, ENABLE);

        data[0] = I2C_ReceiveData(I2C2);
        data[1] = I2C_ReceiveData(I2C2);

        I2C_NACKPositionConfig(I2C2, I2C_NACKPosition_Current); // POS=0
        I2C_AcknowledgeConfig(I2C2, ENABLE);
        return true;
    }
    ...
    // wait for BTF=1: the first two bytes are buffered to DR/SR
    I2C_WAIT_FLAG(I2C_FLAG_BTF, 1000);
    // turn off ACK, and enable NACK
    I2C_AcknowledgeConfig(I2C2, DISABLE);
    ...
    *data++ = I2C_ReceiveData(I2C2);
    ...
    // read the second before final byte
    I2C_WAIT_FLAG(I2C_FLAG_RXNE, 1000);
    *data++ = I2C_ReceiveData(I2C2);
    ...
    *data++ = I2C_ReceiveData(I2C2);
    ...
    while (I2C2->CR1 & I2C_CR1_STOP); // wait until stop recieved
    I2C_AcknowledgeConfig(I2C2, ENABLE);
    return true;
}
...
static bool aht20_read(uint8_t data[], uint32_t length)
{
    ...
    bool ok = i2c_read_bytes(data, length);
    if (!ok) I2C_GenerateSTOP(I2C2, ENABLE);

    // check clk and sda status
    if (!GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_10) || !GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_11)) {
        bus_recover();
        cpu_delay(20 * 1000);
    }
...
static bool aht20_read_status(uint8_t *status)
{
    ...
    bool ok = i2c_read_bytes(status, 1);
    return ok;
}
